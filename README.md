...
In order to perform the face recognition, our algorithm first converts the .pgm images into matrices containing decimal or integers values. The purpose of dealing with matrices is to perform calculation of eigenvectors of the correlation matrix for each image. Note that an eigenvector of a linear transformation changes by only a scalar factor (called eigenvalue) when that linear transformation is applied to a given vector. Therefore, our algorithm will also compute the minimum eigenvalue of each image (matrix) while performing the recognition process.
